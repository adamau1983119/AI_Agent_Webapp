...
> **v7 已凍結（2026-07-29）**：本檔進入**唯讀保全**；完整快照見 [docs/archives/v7.0.0_專案完整架構表_凍結.md](./docs/archives/v7.0.0_專案完整架構表_凍結.md) · Git tag \7.0.0-code-freeze\。  
> **上架／測試／熱修 SoT 改為 v8**：[專案完整架構表_v8.md](./專案完整架構表_v8.md) · 分支 \eature/v8-launch\。  
> **Manifest**：[docs/archives/v7.0.0_CODE_FREEZE_MANIFEST.md](./docs/archives/v7.0.0_CODE_FREEZE_MANIFEST.md)
...
| **開發期 DNS** | **暫不改** Spaceship 記錄；本機 `localhost:3000`／`8000` |
| **建議上線拆分** | 前端 `https://ai-alterego.com`；API `https://api.ai-alterego.com`（可改同域 `/api`） |
| **上線 Checklist** | [`docs/alter_ego_launch_dns_checklist.md`](./docs/alter_ego_launch_dns_checklist.md)（含 Spaceship 記錄列 + `.env` 對照） |
| **OAuth 衍生** | `BACKEND_URL` → Meta／Google **redirect URI**；`FRONTEND_URL` → 郵件連結 |
| **託管方案** | **A** VPS + Nginx；**B** Vercel + Railway（文件附錄 A 二選一） |
...
| `/compose-sample` | ComposeSamplePage | **顯示** | 組裝練習（demo；0 LLM） |
| `/ops/style-trainer` | StyleTrainerPage | **allowlist** | 風格訓練窗；admin／tester（＋可選 email）；analyze／confirm Mongo；解析中右欄 Skeleton |
