...
export default function Channels() {
  ...
  const handleDeleteChannel = async (channelId: string, channelName: string) => {
    if (!confirm(`確定要刪除頻道「${channelName}」嗎？此操作無法復原。`)) {
      return;
    }
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              管理您的自定義內容頻道（{channels.length}/{maxChannels}）
            </p>
          </div>
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-shadow duration-200">
      ...
      <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
        <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
          <span>{channel.topic_count} 個主題</span>
          {channel.last_collected_at && (
            <span>
              上次收集：{new Date(channel.last_collected_at).toLocaleDateString()}
            </span>
          )}
        </div>
