...
| 7 | 頻道 **UI CRUD／幽靈資料** E2E 未封口 | P0 | 第 12～14 天 | 清單 **H**、matrix **§D** | 建立 → 列表可見 → 編輯／刪除（若有 UI）後列表一致；或 **BLOCK** 建 issue | [ ] |
| 8 | 主線通過率未滿（詳情／頻道助手／靈感） | P0 | 第 10～13 天 | `工作記錄` 頂部統計 | 更新 **分數+分母** 與日期；未測項 **≤3** 須註原因（對齊 AGENTS） | [ ] |
