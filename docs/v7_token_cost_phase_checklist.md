...
> **觸發對齊**：對話 **`專案開始`**（預設 v7）→ [`AGENTS.md`](../AGENTS.md)「## 專案開始（v7）」＋本檔；核證勾選同步 [`工作記錄.md`](../工作記錄.md) **§ v7 Token 開發核證（V7）**（**V7-0～V7-4**）。**`專案開始 v6`** 才走 R+T 測試週表。  
...
> **Discover SKU（2026-06-12）**：**[`v7_discover_public_feed_checklist.md`](./v7_discover_public_feed_checklist.md)** — **PD-0～4 程式** `[x]`；**CD-*／E0-PF** 測試段 **06-16～18**（**勿**與本檔 C* 混勾）。  
> **實作基礎守則（改碼前必讀）**：**[`v7_implementation_basics.md`](./v7_implementation_basics.md)**（**BF-***；含 README i18n／按鈕 testid／六必讀門檻）。  
> **Key 輪換（2026-06-13）**：舊 `ai-agent-webapp-production`／`production 2` 已刪；本機新 key 尾碼 **ebe36**（**勿** commit `.env`）。見 [`deepseek_cost_investigation_2026-05.md`](./deepseek_cost_investigation_2026-05.md) **§8**。
...
**證據**：[`docs/evidence/v7/2026-06-10/A-track_agent_session_log.md`](./evidence/v7/2026-06-10/A-track_agent_session_log.md)、[`docs/evidence/v7/2026-06-13/KEY_rotation_session_log.md`](./evidence/v7/2026-06-13/KEY_rotation_session_log.md)

- [x] **DT-1** 雙軌政策寫入本檔 + 工作記錄  
- [x] **DT-2** A 軌 2026-06-10 已跑一輪（build／health／C3-3 靜態）  
- [ ] **DT-3** U 軌用家煙霧（5 步）— 建議併 **06-16** 或 commit 前  
...
> **未完成的 E0 不得勾選當日任何依賴本機服務的 C*-*。** 證據檔名見截圖指南 §3。
...
- [x] **P5-01** Privacy Policy / APPI 跨境披露（法務）  
  - 證據：`Privacy.tsx` + i18n §7（DeepL／DeepSeek／APPI）；`V7-5_Phase5_session_log.md` — **草稿**待法務覆核
