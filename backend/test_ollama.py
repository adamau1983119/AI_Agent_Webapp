...
async def test_ollama_generation():
    ...
    try:
        ...
        try:
            ...
            if article:
                logger.info(f"內容預覽: {article[:300]}...")
            else:
                logger.warning("⚠️ 回應內容為空，請檢查 API 回應格式")
        except Exception as e:
            ...
            if settings.OLLAMA_API_KEY:
                logger.info("1. API Key 是否正確")
                logger.info("2. 網路連接是否正常")
                logger.info("3. Ollama 雲端服務是否可用")
            else:
