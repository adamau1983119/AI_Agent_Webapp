...
async def create_food_topic():
    """建立美食推介主題範例：2026年台北米其林餐廳推薦"""
    topic_repo = TopicRepository()
    
    topic_data = {
        "id": "taipei_michelin_2026",
        "title": "2026年台北米其林餐廳推薦",
        "category": Category.FOOD,  # 美食分類
        ...
        "sources": [
            {
                ...
                "type": "official",
                "keywords": ["米其林", "台北", "餐廳", "美食", "fine dining", "Michelin", "Taipei", "restaurant"],
                "verified": True,
            ...
            },
            {
                ...
                "type": "food_platform",
                "keywords": ["OpenRice", "台北", "餐廳推薦", "美食", "restaurant review"],
                "verified": True,
                "verified_at": datetime.utcnow().isoformat(),
                "reliability": "high"
            }
...
async def generate_content_for_food():
    ...
    logger.info(f"📝 開始生成美食內容，關鍵字: {keywords}")
...
async def search_images_for_food():
    ...
    # 美食相關關鍵字
    keywords = "台北 米其林 餐廳 美食 fine dining Taipei Michelin restaurant"
    
    logger.info(f"🖼️ 開始搜尋美食圖片，關鍵字: {keywords}")
...
async def main():
    ...
    try:
        ...
        logger.info("\n" + "=" * 60)
        logger.info("✅ 美食推介主題建立完成！")
        logger.info("=" * 60)
        logger.info(f"📄 結果已儲存到: food_sample_result.json")
