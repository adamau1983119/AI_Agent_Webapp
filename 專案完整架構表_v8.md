...
> **更新日期**：2026-09-17（**Style Trainer PR #72**＋**Compose 簡化 PR #73** 合 `main`；Trainer analyze Skeleton `feature/v8-trainer-analyze-skeleton`）  
> **分支**：正式域映像追 `main`（PR #73 組裝簡化、PR #72 Style Trainer、PR #68／#69 組裝進階、PR #67 Welcome 價錢、PR #65 痛點、PR #64 金流履約、PR #62 精選照片、PR #59 金流、PR #58 組裝器）；Skeleton 見 `feature/v8-trainer-analyze-skeleton`（`5c0843d`）  
...
|------|------------|------------------------|
| 程式基線 | `9556ea7` 已合 `main`（PR #17） | OBS＋…＋**PR #58 組裝器**＋**PR #59／#64 金流**＋**PR #62 精選照片**＋**PR #65 痛點**＋**PR #67 Welcome 價錢**＋**PR #68／#69 Compose advanced**＋**PR #72 Style Trainer**＋**PR #73 Compose 簡化（IG／FB、短500／長1500）**＋**問題卡 hide** |
...
| **主題詳情排版** | 舊版垂直堆疊 | **手繪草圖 1:1 重構**（大圖＋源文＋左圖右組裝器）；**v8 主路** `PostComposerPanel`（**僅 FB／IG**；**短≤500（−1）／長≤1500（−2）**；Threads／Keep／intent **UI 隱藏**；`domain`→trainer few-shot）；舊短文面板／Post Kit **不掛載、檔案保留** |
| **風格訓練（ops）** | 無 | **PR #72**：`/ops/style-trainer`（admin／tester＋可選 `OPS_STYLE_TRAINER_EMAILS`）；analyze／confirm → Mongo `style_trainer_exemplars`；compose 結構＋few-shot（Mode A／B）。**Skeleton**：解析中右欄灰條（`feature/v8-trainer-analyze-skeleton`） |
...
| **公眾卡正文** | RSS／頁面雜訊進譯文 | **全站** `article_boilerplate`：刪分享列；`選購*`／購物／影片／相關閱讀／Newsletter **整段截斷**（不依來源特例）；詳情 GET **顯示層**再清一次（不回寫 Mongo） |
| **公眾卡出文** | Post Kit FB/Threads/X 仿文 | **PR #58＋#68／#69＋#73**：`POST /alter-ego/compose` Flash；**僅 FB／IG**；短 **≤500（−1）**／長 **≤1500（−2）**；`part=all` body→meta；Hashtag **3～5**；tone card；`domain` 對齊 trainer；Threads／Keep **UI 隱藏**（鍵／檔案保留）；示範 `/compose-sample`（0 LLM／0 扣點） |
...
| `/topics/:id` | TopicDetail | **顯示** | 含 translate-display、按需 translate-source-article、**社群發文組裝器**（**FB／IG**；短500／長1500）；舊 Post Kit／短文面板不掛載；`btn-topic-detail-back` 回頭條 |
...
| `/ops/style-trainer` | StyleTrainerPage | **allowlist** | 風格訓練；admin／tester＋可選 email；analyze／confirm Mongo；解析中右欄 Skeleton |
...
**v8 現行主路（PR #58＋#68／#69＋#73）**：TopicDetail → `PostComposerPanel` → `POST /api/v1/alter-ego/compose`（**僅 FB／IG**；**短≤500（−1）／長≤1500（−2）**；`part=all` 兩段 body→meta；`domain`→trainer few-shot）。示範頁 `/compose-sample`（0 LLM）。`preview`／`contents/generate` 仍可用、不扣點。Threads／Keep **UI 隱藏**。

**Ops 風格訓練（PR #72）**：Sidebar → `/ops/style-trainer` → `POST /api/v1/ops/trainer/analyze|confirm` → Mongo `style_trainer_exemplars` → compose 注入。
...
| `POST /api/v1/alter-ego/compose` | Flash | 公眾卡發文套件（**FB／IG**；短500／長1500；`part=all\|body\|…`；body→meta；`domain`；成功短−1／長−2） | ✅ **PR #58／#68／#69／#73** |
| `GET/POST /api/v1/ops/trainer/*` | Flash／Vision | access／coverage／analyze／confirm；Mongo 範例；allowlist | ✅ **PR #72** |
...
```
┌────────────────────────────────────────────────────────────────────────┐
│ 1. RSS / 採集端 (ArticleExtractor + article_boilerplate · 全站同一出口) │
│    ├── HTTP／RSS content:encoded 抽取正文                             │
│    ├── 刪分享／廣告／圖片來源等 chrome 行                             │
│    ├── 整行結束標記截斷：購物／最新影片／相關閱讀／Newsletter／作者欄   │
│    └── ➔ original_content = 報導正文（最高 5000 字）                 │
└───────────────────────────────────┬────────────────────────────────────┘
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
...
│   │   └── automation/                           # 定向預載 D3 + public_feed_batch 8h + topic_card_finalize ✅
...
│       ├── alter_ego.py      # extract/preview/status/compose ✅
│       ├── ops_trainer.py  # Style Trainer access/coverage/analyze/confirm ✅
...
│   ├── pages/StyleTrainerPage.tsx                # Ops 風格訓練；analyze Skeleton ✅
│   ├── data/composeDemoPack.ts                   # canned demo pack 三語 ✅
│   ├── api/opsTrainer.ts                         # ops trainer client ✅
│   ├── components/credits/CreditPackShelf.tsx    # 點數三卡；landing 價錢 Inter／settings Cormorant ✅
│   ├── components/features/PostKitPanel.tsx      # 發文套件 L0 ✅（詳情主路不掛載）
│   ├── components/features/PostComposerPanel.tsx # 公眾卡組裝器 **FB／IG**；短500／長1500 ✅
...
│   ├── compose_service.py / compose_generate.py  # JIT 組裝＋body→meta＋domain few-shot ✅
│   ├── compose_caps.py / compose_tone.py         # LENGTH 500／1500；credit 1／2 ✅
│   ├── compose_prompt.py / compose_parse.py      # 事實錨＋structure／few-shot ✅
│   ├── ops_trainer_*.py                          # analyze／store／inject／access ✅
│   ├── ai/deepseek.py                            # generate_multimodal（ops Vision）✅
│   └── …（其餘服務略）
│
├── docs/
...
| 版本 | 日期 | 更新 |
|------|------|------|
| **v8.0.10-trainer-skeleton** | 2026-09-17 | Trainer 右欄 analyze Skeleton；`feature/v8-trainer-analyze-skeleton` `5c0843d`（待合） |
| **v8.0.9-compose-simplify** | 2026-09-17 | **PR #73**：組裝器僅 FB／IG；短≤500（−1）／長≤1500（−2）；藏 Threads／Keep |
| **v8.0.8-ops-style-trainer** | 2026-09-17 | **PR #72**：`/ops/style-trainer`＋Mongo 範例＋compose few-shot／Vision multimodal |
