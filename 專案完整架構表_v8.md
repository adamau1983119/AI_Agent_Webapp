...
> **更新日期**：2026-09-04（**PR #59 金流合 `main`** `72b18a0`；正式域 02:52 UTC 重啟 15/15；精選照片 4 張仍在 `feature/v8-featured-photos` 未合）  
> **分支**：正式域映像追 `main`（PR #59 金流 `72b18a0`、PR #58 組裝器 `d5457dc`、PR #57 分類 Tab `a2fb8bf`、PR #56 Dashboard IA `90acb34`、PR #52 正文截斷 `7d55dc3`、PR #51 產出契約 `2f6607e`）  
...
| **主題詳情排版** | 舊版垂直堆疊 | **手繪草圖 1:1 重構**；**v8 主路** `PostComposerPanel`（FB/IG/Threads、50/100/150、−1 點 JIT）；舊短文／Post Kit **不掛載、檔案保留** |
...
| **公眾卡出文** | Post Kit FB/Threads/X 仿文 | **PR #58**：`POST /alter-ego/compose` Flash；成功扣 1 點；Threads cap **150** |
| **點數／金流** | 一次發 5 點、無結帳 | **PR #59**：歡迎 3×10 後每日 +5（免費帽 10、7 日到期）；購買 US$3/5/10 不過期；Mongo wallet SoT；無 Stripe 金鑰 **503** |
| **精選照片** | 匹配綁長文、預設湊 8 | **程式在 `feature/v8-featured-photos`（最多 4 張 2×2）— 2026-09-04 尚未合 `main`** |
| **實體硬門禁** | 手動測試 | **Git Pre-Commit 5 道實體門禁**（結構 + i18n **1137** 鍵 + 管線審計 + 語言 14 項 + 防線／抽取含 compose） |
...
| `POST /api/v1/alter-ego/compose` | Flash | 公眾卡發文套件（FB/IG/Threads；成功後扣 1 點） | ✅ |
...
| **v8.0.2-post-composer** | 2026-09-03 | **PR #58**：詳情主路 `PostComposerPanel` + `POST /alter-ego/compose` |
