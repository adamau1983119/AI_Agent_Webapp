...
> **更新日期**：2026-09-16（**問題卡 hide** `feature/v8-topic-hide-feed`；Compose advanced 已合 `main` PR #68＋Py3.11 hotfix #69；Welcome 價錢 Inter **PR #67**）  
> **分支**：正式域映像追 `main`（PR #68／#69 組裝進階、PR #67 Welcome 價錢、PR #65 痛點、PR #64 金流履約、PR #62 精選照片、PR #59 金流、PR #58 組裝器）；**問題卡 hide** 見 `feature/v8-topic-hide-feed`（`e26b9c6`）  
...
|------|------------|------------------------|
| 程式基線 | `9556ea7` 已合 `main`（PR #17） | OBS＋PR #19 Resend＋PR #24 HKT＋PR #38/39 搜圖＋**事實錨定／源文章翻譯**＋**PR #51 產出契約**＋**PR #52 正文截斷**＋**PR #58 組裝器**＋**PR #59／#64 金流**＋**PR #62 精選照片 4 張**＋**PR #65 痛點 1～4**＋**PR #67 Welcome 價錢 Inter**＋**PR #68／#69 Compose advanced**＋**問題卡 hide（feature/v8-topic-hide-feed）** |
...
| **主題詳情排版** | 舊版垂直堆疊 | **手繪草圖 1:1 重構**（大圖＋源文＋左圖右組裝器）；**v8 主路** `PostComposerPanel`（FB/IG/Threads、**100／150／500**、Keep／草稿／intent、−1 點 JIT）；舊短文面板／Post Kit **不掛載、檔案保留** |
...
| **公眾卡出文** | Post Kit FB/Threads/X 仿文 | **PR #58**＋**Compose advanced（待合）**：`POST /alter-ego/compose` Flash；`part=all` **兩段 body→meta**；成功扣 1 點；tone card；Hashtag **3～5**（Threads **1～5**）；Threads 產品 cap **150**（無 500）；`preserve_snippets`／`revision_intent`；DNA 語氣 overlay 可選（不注入 lexicon）；示範 `/compose-sample`（0 LLM／0 扣點） |
...
| `/topics/:id` | TopicDetail | **顯示** | 含 translate-display、按需 translate-source-article、**社群發文組裝器**（舊 Post Kit／短文面板不掛載、檔案保留）；`btn-topic-detail-back` 回頭條 |
