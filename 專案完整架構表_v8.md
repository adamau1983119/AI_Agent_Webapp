...
> **更新日期**：2026-09-16（**Welcome 點數價錢 Inter** 於 `feature/v8-welcome-price-font` `1e88287`；痛點 PR #65 已合 `main` `cd5a891`）  
> **分支**：正式域映像追 `main`（PR #65 痛點 `cd5a891`、PR #64 金流履約 `fcf70b5`、PR #62 精選照片、PR #59 金流、PR #58 組裝器）；Welcome 價錢字體見 `feature/v8-welcome-price-font`（待合）  
...
|------|------------|------------------------|
| 程式基線 | `9556ea7` 已合 `main`（PR #17） | OBS＋PR #19 Resend＋PR #24 HKT＋PR #38/39 搜圖＋**事實錨定／源文章翻譯**＋**PR #51 產出契約**＋**PR #52 正文截斷**＋**PR #58 組裝器**＋**PR #59／#64 金流**＋**PR #62 精選照片 4 張**＋**PR #65 痛點 1～4**＋**Welcome 價錢 Inter（feature 待合）** |
...
| **主題詳情排版** | 舊版垂直堆疊 | **手繪草圖 1:1 重構**（大圖＋源文＋左圖右組裝器）；**v8 主路** `PostComposerPanel`（FB/IG/Threads、50/100/150、−1 點 JIT）；舊短文面板／Post Kit **不掛載、檔案保留** |
...
| **公眾卡出文** | Post Kit FB/Threads/X 仿文 | **PR #58**：`POST /alter-ego/compose` Flash；成功扣 1 點；風格進 Prompt；Threads cap **150**；DNA 語氣 overlay 可選（不注入 lexicon） |
| **點數／金流** | 一次發 5 點、無結帳 | **PR #59／#64**：歡迎 3×10 後每日 +5（免費帽 10、7 日到期）；購買 US$3/5/10 不過期；Mongo wallet SoT；無 Stripe 金鑰 **503**。**UI**：`CreditPackShelf` 共用 Welcome／Settings；Welcome 價錢主列 **Inter**（`font-price`，ChatGPT 數字感）；Settings 仍 Cormorant `font-display` |
| **實體硬門禁** | 手動測試 | **Git Pre-Commit 5 道實體門禁**（結構 + i18n **1162** 鍵 + 管線審計 + 語言／防線／抽取） |
...
**v8 現行主路（PR #58）**：TopicDetail → `PostComposerPanel` → `POST /api/v1/alter-ego/compose`（FB／IG／Threads；成功 −1 點）。`preview`／`contents/generate` 仍可用、不扣點。
...
| `POST /api/v1/alter-ego/compose` | Flash | 公眾卡發文套件（FB/IG/Threads；成功後扣 1 點） | ✅ **PR #58**／正式域已部署 |
...
│   ├── components/features/PostComposerPanel.tsx # 公眾卡組裝器 FB/IG/Threads ✅
...
| 版本 | 日期 | 更新 |
|------|------|------|
| **v8.0.5b-welcome-price-font** | 2026-09-16 | Welcome 點數價錢 **Inter**（`font-price`）；Settings 仍 Cormorant；`1e88287`／`feature/v8-welcome-price-font`（待合） |
