...
> **更新日期**：2026-09-03（**公眾卡選文閘門** `selection.mode` 預設 off；產出契約 PR #51／正文截斷 #52）  
> **分支**：正式域映像追 `main`（選文 `feature/v8-topic-card-select` `f07053c`、PR #52 正文截斷 `7d55dc3`、PR #51 產出契約 `2f6607e`、PR #39 image `6fa295d`、PR #38 dedup `4819e99`）  
...
|------|------------|------------------------|
| 程式基線 | `9556ea7` 已合 `main`（PR #17） | OBS＋PR #19 Resend＋PR #24 HKT＋PR #38/39 搜圖＋**事實錨定／源文章翻譯**＋**PR #51 產出契約**＋**PR #52 正文截斷**＋**選文閘門**（`f07053c`，預設 off） |
| 測試 | 上架衝刺三日主路已過 | **正式域日常運作**（CX／OPS）；**5 道實體硬門禁 100% 守護** |
| 託管 | 未接 | **方案 B**：Vercel + Railway ✅ |
| DNS／OAuth | 未接 | 前端＋API DNS／health ✅；Resend 網域 Verified ✅ |
| 產卡 | 預設關 | **`ENABLE_SCHEDULED_TOPIC_COLLECTION=true`**（每日 **04:00 HKT** × 各 5 = **15**，`generate_content=false`）；**選文** `selection.mode: off`（shadow／enforce 可開） |
...
| **公眾卡選文** | RSS 第一則即入庫（分數未用於挑選） | **標題層閘門**（scrape／Flash／翻譯前）：`off`＝舊路徑；`shadow`＝入庫＋would_skip 日誌；`enforce`＝分類錨點＋來源上限，不足則 **fail-open** 舊路徑補滿 |
...
| **產卡排程** | ✅ collection 開；`topic_generation.yaml` **daily 04:00 HKT** × 三類各 5；**選文** `selection.mode: off` |
...
| 選文 | `automation/topic_card_select_config.py`、`topic_card_gates.py`、`topic_card_select.py` |
| 前端 | `frontend/src/lib/topicDisplay.ts`、`TopicTranslateDisplayButton.tsx`、`TopicCard.tsx`、`TopicDetail.tsx` |

---

## 📰 主題卡源文章完整新聞翻譯與事實錨定架構（2026-08-20；**2026-09-02** 產出契約；**2026-09-03** 選文閘門）
...
```
┌────────────────────────────────────────────────────────────────────────┐
│ 0. 選文閘門 (selection.mode · 預設 off＝舊 RSS 第一則路徑)              │
│    off：不篩選                                                        │
│    shadow：舊路徑入庫 + TOPIC_CARD_SELECT would_skip 日誌               │
│    enforce：分類錨點＋優惠券 URL＋同來源上限；不足 C 張 fail-open 補滿 │
└───────────────────────────────────┬────────────────────────────────────┘
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
...
│   │   └── automation/                           # 定向預載 D3 + public_feed_batch 8h + topic_card_finalize + 選文閘門 ✅
...
| 公眾卡選文 | `backend/app/services/automation/topic_card_*.py` | yaml `selection.mode`：off／shadow／enforce；預設 off；enforce fail-open |
...
**流水線日誌前綴**：`[SUMMARY_FLASH_SUCCESS]`、`[I18N_CACHE_HIT]`、`[CACHE_MISS]`、`[TRANSLATION_FALLBACK_TRIGGERED]`、`[TOKEN_GATEWAY_PASSED]`、`[PUBLIC_FEED_DEV_CAP]`、`TOPIC_CARD_SELECT`（shadow／enforce）
...
| 版本 | 日期 | 更新 |
|------|------|------|
| **v8.0.2-topic-select** | 2026-09-03 | 公眾卡標題選文閘門（`selection.mode` 預設 off；shadow／enforce＋fail-open）；未改翻譯管線 |
