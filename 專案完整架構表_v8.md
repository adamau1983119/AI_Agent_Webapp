...
> **更新日期**：2026-09-04（**PR #62 精選照片 4 張 2×2 合 `main`** `1aaf098`；金流 PR #59 已在正式域）  
> **分支**：正式域映像追 `main`（PR #62 精選照片 `1aaf098`、PR #59 金流 `72b18a0`、PR #61 文件 `5cd5583`、PR #58 組裝器 `d5457dc`、PR #57 分類 Tab `a2fb8bf`、PR #56 Dashboard IA `90acb34`、PR #52 正文截斷 `7d55dc3`、PR #51 產出契約 `2f6607e`）  
...
|------|------------|------------------------|
| 程式基線 | `9556ea7` 已合 `main`（PR #17） | OBS＋PR #19 Resend＋PR #24 HKT＋PR #38/39 搜圖＋**事實錨定／源文章翻譯**＋**PR #51 產出契約**＋**PR #52 正文截斷**＋**PR #58 組裝器**＋**PR #59 金流**＋**PR #62 精選照片 4 張** |
...
| **源文章全文翻譯** | 僅 30 字摘要 | **產出時** `finalize_produced_cards` 寫入三語 `source_content_i18n`；GET **僅 overlay 快取**（不重抓、不 Flash）。DeepSeek Flash 新聞級譯文＋前端原文／譯文對照 |
...
| **精選照片** | 匹配綁長文、預設湊 8 | **PR #62**：最多 4 張 2×2；事實錨 `summary_flash`→原文→標題；空庫可匹配；不改產卡 `preview_images: 1` |
| **主題卡去重** | — | PR **#38**：補生成只填分類缺口；Dashboard 標題去重 |
| **公眾卡正文** | RSS／頁面雜訊進譯文 | **全站** `article_boilerplate`：刪分享列；購物／影片／相關閱讀／Newsletter **整段截斷**（不依來源特例） |
...
| **實體硬門禁** | 手動測試 | **Git Pre-Commit 5 道實體門禁**（結構 + i18n **1139** 鍵 + 管線審計 + 語言 14 項 + 防線／抽取含 compose） |
...
| **精選上限** | 畫廊／匹配／新增合計 **最多 4 張、2×2**；智能匹配事實＝`summary_flash`→原文→標題（不需長文）；產卡 `preview_images: 1` **不改** |
...
| `/topics/:id` | TopicDetail | **顯示** | 含 translate-display、**社群發文組裝器**（舊 Post Kit／短文面板不掛載、檔案保留）；`btn-topic-detail-back` 回頭條 |
...
│   ├── utils/article_boilerplate.py      # 全站正文截斷／chrome 清除 ✅
│   ├── services/
