...
> **更新日期**：2026-09-18（**抽取品質 P0** `feature/v8-popbee-extract-quality` `d03fc7b` 待合；**PR #75** 列表 sort 已合 `main`）  
> **分支**：正式域映像追 `main`（**PR #75** sort_penalty 不埋今日卡；PR #68／#69 組裝進階、PR #67 Welcome 價錢、PR #65 痛點、PR #64 金流履約、PR #62 精選照片、PR #59 金流、PR #58 組裝器）；**抽取品質** 見 `feature/v8-popbee-extract-quality`（⏳ 待合）；**問題卡 hide** 見 `feature/v8-topic-hide-feed`（`e26b9c6`）  
...
> **日收備份**：[2026-09-04 精選照片 PR #62](./docs/backups/2026-09-04_featured-photos_snapshot/SNAPSHOT_README.md) · [2026-09-04 金流 PR #59](./docs/backups/2026-09-04_credits-kling_snapshot/SNAPSHOT_README.md) · [2026-09-02 產出契約＋正文截斷](./docs/backups/2026-09-02_topic-card-cx_snapshot/SNAPSHOT_README.md) · [2026-08-20 源文章全文翻譯與事實錨定門禁](./docs/backups/2026-08-20_source-article-translation-and-guardrails_snapshot/SNAPSHOT_README.md) · [2026-08-12 Railway env／圖片搜尋](./docs/backups/2026-08-12_railway_env_image_ops_snapshot/SNAPSHOT_README.md) · [2026-08-11 topic HKT PR24](./docs/backups/2026-08-11_topic_dashboard_hkt_pr24_snapshot/SNAPSHOT_README.md) · [2026-08-11 ops digest](./docs/backups/2026-08-11_ops_digest_resend_pass_snapshot/SNAPSHOT_README.md)  
...
|------|------------|------------------------|
| 程式基線 | `9556ea7` 已合 `main`（PR #17） | OBS＋PR #19 Resend＋PR #24 HKT＋PR #38/39 搜圖＋**事實錨定／源文章翻譯**＋**PR #51 產出契約**＋**PR #52 正文截斷**＋**PR #58 組裝器**＋**PR #59／#64 金流**＋**PR #62 精選照片 4 張**＋**PR #65 痛點 1～4**＋**PR #67 Welcome 價錢 Inter**＋**PR #68／#69 Compose advanced**＋**PR #75 列表 sort**＋**問題卡 hide**＋**抽取品質 P0（待合）** |
| 測試 | 上架衝刺三日主路已過 | **正式域日常運作**（CX／OPS）；**5 道實體硬門禁 100% 守護** |
| 託管 | 未接 | **方案 B**：Vercel + Railway ✅ |
| DNS／OAuth | 未接 | 前端＋API DNS／health ✅；Resend 網域 Verified ✅ |
| 產卡 | 預設關 | **`ENABLE_SCHEDULED_TOPIC_COLLECTION=true`**（每日 **04:00 HKT** × 各 5 = **15**，`generate_content=false`）；**寬門**：抽壞仍建卡（標題＋flash＋外鏈） |
| Dashboard 今日卡 | UTC 日 filter（bug） | **HKT 營運日**（`topicDayHkt.ts`／`topic_day_hkt.py`）；列表排序 **`generated_at` 先、`sort_penalty` 僅 tie-break**（PR #75） |
| 三語預載 | 按需 translate-display | **可選** `ENABLE_TOPIC_TRIPLE_PRELOAD`（DeepL en/ja 產後批次） |
| **源文章全文翻譯** | 僅 30 字摘要 | **產出時** `finalize_produced_cards` 寫入三語 `source_content_i18n`；GET **僅 overlay**（不重抓、不 Flash）。缺譯文時 UI 標**事實摘要**（不冒充報道）；**按需** `POST …/translate-source-article`（既有 `on_demand=True`） |
| **事實錨定與防污染** | 詞庫直塞 prompt | **規則 19 鐵律**：Soul/Article Prompt 事實錨定（Fact Grounding）與風格解耦（Anti-Pollution）；移除背景自動偷跑，嚴格按需出文 (JIT) |
| **主題詳情排版** | 舊版垂直堆疊 | **手繪草圖 1:1 重構**（大圖＋源文＋左圖右組裝器）；**v8 主路** `PostComposerPanel`（FB/IG、短≤500／長≤1500、−點 JIT）；舊短文面板／Post Kit **不掛載、檔案保留**；壞正文→**flash 回退**＋`topics.sourceExtractFallback` |
...
| **精選照片** | 匹配綁長文、預設湊 8 | **PR #62**：最多 4 張 2×2；事實錨 `summary_flash`→原文→標題；空庫可匹配；不改產卡 `preview_images: 1`。**痛點**：主圖 `object-top`；計數 `min(n,4)/4`；縮圖／預覽**下載 JPG**（proxy＋canvas，不改匹配） |
| **主題卡去重** | — | PR **#38**：補生成只填分類缺口；Dashboard 標題去重 |
| **公眾卡正文** | RSS／頁面雜訊進譯文 | **寬門＋審查沉底**：`article_body_resolve` **RSS 優先於 HTTP**；`article_extract_quality` 拒殼當 success；`article_boilerplate` mega-menu≥4／CTA／分享列／recirc；產後 `topic_post_scan`→`content_clean`＋高 `sort_penalty`（**不拒產**；嚴重付費牆才 hide）；詳情壞文→`summary_flash`；INFO：`TOPIC_EXTRACT_*`／`TOPIC_CARD_SINK` |
| **公眾卡出文** | Post Kit FB/Threads/X 仿文 | **PR #58**＋**Compose simplify**：`POST /alter-ego/compose` Flash；平台 **FB／IG**；長度短≤500／長≤1500；成功扣點；DNA 語氣 overlay 可選；示範 `/compose-sample`（0 LLM／0 扣點） |
| **點數／金流** | 一次發 5 點、無結帳 | **PR #59／#64**：歡迎 3×10 後每日 +5（免費帽 10、7 日到期）；購買 US$3/5/10 不過期；Mongo wallet SoT；無 Stripe 金鑰 **503**。**UI**：`CreditPackShelf` 共用 Welcome／Settings；Welcome 價錢主列 **Inter**（`font-price`）；Settings 仍 Cormorant；卡內 **✓／✗ 權益列**（產量／單次價誠實差異＋堆疊句） |
| **實體硬門禁** | 手動測試 | **Git Pre-Commit 5 道實體門禁**（結構 + i18n **1210** 鍵 + 管線審計 + 語言／防線／抽取） |
| 文件 SoT | archives 凍結檔 | **本檔** |
| **上架判決** | — | **已上架營運**；OPS-DIGEST **PASS**；FIX-TOPIC **PR #24 merged** |
| 護城河 / GTM | 技術競爭（泛用） | **工作流/信任/反饋迴路 + 一站式低摩擦**；Google Ads **日本主場**、台港副；金流現為 Stripe **一次過三檔**（B-auto 月訂後補） |
...
| **精選上限** | 畫廊／匹配／新增合計 **最多 4 張、2×2**；智能匹配事實＝`summary_flash`→原文→標題（不需長文）；產卡 `preview_images: 1` **不改**；UI 計數 `min(n,4)/4`；主圖 `object-top`；下載 JPG＝`downloadJpeg.ts`（不改庫 URL／匹配） |
...
| `/dashboard` | Dashboard | **顯示** | **v8 登入預設首屏**；全部＝三類今日各 5＋更多切 `?tab=`；分類 Tab 直接今日＋歷史（隱藏分類「更多」）；HKT N/15 |
| `/topics` | Topics | **隱藏** | 側欄不顯示；路由保留（Header 搜尋／空狀態）；`?category=` 仍可用 |
| `/topics/:id` | TopicDetail | **顯示** | translate-display、按需 translate-source-article、組裝器；壞正文→`summary_flash`＋`topics.sourceExtractFallback`；`btn-topic-detail-back` 回頭條 |
...
| `/compose-sample` | ComposeSamplePage | **顯示** | 組裝練習／示範（canned pack；**不**呼叫 compose API、**不**扣點）；側欄 `link-sidebar-compose-sample` |
| `/preferences` | Preferences | **隱藏** | 側欄不顯示；路由／API 保留 |
| `/settings` | Settings | **顯示** | Header 選單 |
| `/schedule` | Schedule | **隱藏** | v7 無排程承諾 |
| `/style-profile` | StyleProfile | **隱藏** | |
| `/social-connect` | SocialConnect | **即將推出** | 側欄徽章 `feature.comingSoon`；路由保留 |
| `/publish` | Publish | **即將推出** | L0 **發布助手**（預設無 API 代發）；`VITE_ENABLE_API_PUBLISH=true` 才顯示舊代發 |
| `/welcome` | Welcome | **引流** | Landing 行銷導流 ✅（`Welcome.tsx`；i18n `landing.*`；testid 頻道區塊 2.0） |
| `/discover` | Discover | **隱藏** | 側欄不顯示；路由／API 保留；**非**登入預設；**僅** `GET …/public/topics/feed` |
...
| **初始點數** | 歡迎 **3×10**（舊戶 `initial_grant` 先 +5 補滿 10）後每日登入 **+5**；免費未到期合計 **最多 10**、**7 日到期**；購買點不過期。Stripe 一次付 **US$3=180／US$5=350／US$10=800**（無訂閱） |
...
| 代號 | 必過 | Checklist | 狀態 |
|------|------|-----------|------|
| **MC-1** | Mongo wallet SoT + 登入發放 + AddPoints | CD-MC1-* | ✅ PD；**PR #59** 取代一次 5 點 |
...
alter_ego_dna + alter_ego_dna_snapshots (bump version_id)
       ...
       │
       ▼
Post Kit（平台切换 FB/Threads/X + Visual Prompt）← 檔案保留；詳情主路不掛載
```

**v8 現行主路（PR #58＋Compose advanced）**：TopicDetail → `PostComposerPanel` → `POST /api/v1/alter-ego/compose`（FB／IG／Threads；**100／150／500**；`part=all` 兩段 body→meta；Keep／intent；成功 −1 點）。示範頁 `/compose-sample`（0 LLM）。`preview`／`contents/generate` 仍可用、不扣點。
...
| `POST /api/v1/alter-ego/compose` | Flash | 公眾卡發文套件（FB/IG/Threads；`part=all\|body\|title\|hashtags\|meta`；body→meta；`preserve_snippets`／`revision_intent`；成功後扣 1 點） | ✅ **PR #58**；進階 **`140e51c` 待合** |
...
| `GET /api/v1/billing/balance` | — | 登入發放後餘額（free+purchased） | ✅ |
| `GET /api/v1/billing/packs` | — | US$3／5／10 點數包 | ✅ |
| `POST /api/v1/billing/checkout` | — | Stripe Checkout；未設金鑰 **503** | ✅ |
| `POST /api/v1/billing/webhook` | — | `checkout.session.completed` 入已購桶 | ✅ |
...
backend/app/api/v1/my_channel.py                  # feed + unlock + admin credits ✅
backend/app/api/v1/billing.py                      # packs + checkout + webhook ✅
backend/app/services/credit_ledger_service.py     # 點數帳本（wallet SoT）✅
backend/app/services/credits/                     # grants／lots／Stripe packs ✅
...
| `POST /api/v1/topics/{id}/translate-source-article` | 允許；按需；GET overlay 不變 |
...
| 前端 | `frontend/src/lib/topicDisplay.ts`、`TopicTranslateDisplayButton.tsx`、`TopicCard.tsx`、`TopicDetail.tsx`、`downloadJpeg.ts`、`ImageGallery.tsx`／`ImagePreview.tsx` |

---

## 📰 主題卡源文章完整新聞翻譯與事實錨定架構（2026-08-20；**2026-09-02** 改產出時契約）
...
```
┌────────────────────────────────────────────────────────────────────────┐
│ 1. RSS / 採集端（寬門 · 通用規則 · 不依站別硬編碼）                      │
│    ├── article_body_resolve：RSS content:encoded／summary 優先於 HTTP   │
│    ├── ArticleExtractor + article_boilerplate（mega-menu≥4／CTA／分享） │
│    ├── article_extract_quality：殼不當 success 正文（仍建卡）           │
│    └── ➔ original_content 或空；標題＋連結必在                           │
└───────────────────────────────────┬────────────────────────────────────┘
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│ 1b. 產後 scan (topic_post_scan · TopicCollector／ChannelCollector)     │
│    ├── content_clean；殼／雜訊 → 高 sort_penalty 沉底（預設不 hide）    │
│    ├── 可見下限 topic_visible_floor；嚴重付費牆且近空才 hide           │
│    ├── 再 Flash → summary_flash（輸入優先 clean）                       │
│    └── INFO：TOPIC_EXTRACT_*／TOPIC_POST_SCAN／TOPIC_CARD_SINK         │
└───────────────────────────────────┬────────────────────────────────────┘
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│ 2. 產出時契約 (finalize_produced_cards · 04:00 HKT／手動 generate-today) │
│    ├── summary_flash、titles_i18n／description_i18n／source_content_i18n │
│    ├── pipeline_version 戳記；三語腳本檢驗不削弱                       │
│    └── ➔ 寫入 Mongo；My Channel 之後接同一契約（本輪不改頻道收集）     │
└───────────────────────────────────┬────────────────────────────────────┘
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│ 3. 主題詳情 GET /api/v1/topics/{id}?lang=…                             │
│    ├── 僅 overlay 已產快取 source_content_i18n（不重抓、不 Flash）     │
│    └── 顯示層 apply_display_clean_to_topic（不回寫 Mongo）             │
└───────────────────────────────────┬────────────────────────────────────┘
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│ 4. 前端展示與事實對照 (TopicDetail.tsx)                                 │
│    ├── 📖 源文章：有譯文才當報道；否則標事實摘要（不冒充譯文）         │
│    ├── ⚠️ 壞／殼正文 → 回退 summary_flash＋topics.sourceExtractFallback │
│    ├── 🔄 原文/譯文切換；按需「翻譯報道」→ translate-source-article    │
│    ├── 🖼️ 主圖 object-top；精選計數 min(n,4)/4；縮圖下載 JPG           │
│    ├── 🔗 原始出處外鏈                                                  │
│    └── ⚓ 組裝器事實錨：summary_flash／原文（PostComposerPanel）       │
└────────────────────────────────────────────────────────────────────────┘
```
...
| Gate 4 | 多語言腳本轉換與主題卡／抽取單元測試 (14+22) | `python -m unittest backend/tests/test_i18n_language_conversion.py backend/tests/test_ops_alert_card_fix.py` |
...
│   ├── utils/article_boilerplate.py      # 全站正文截斷／chrome 清除＋顯示層清洗 ✅
│   ├── utils/article_extract_quality.py  # 殼／CJK／mega 品質閘（不擋建卡）✅
│   ├── utils/article_body_resolve.py     # RSS 優先於 HTTP 正文解析 ✅
│   ├── utils/article_extractor.py        # HTML／RSS 抽取＋品質拒殼 ✅
│   ├── services/
...
│   │   ├── translation/source_article_translator.py  # 源文報道 on_demand ✅
...
│   │   └── automation/                           # 定向預載 D3 + public_feed_batch 8h + topic_card_finalize + topic_post_scan ✅
...
│   └── api/v1/
│       ├── topics.py       # translate-display + translate-source-article ✅
...
├── frontend/src/
│   ├── app/App.tsx
│   ├── pages/Welcome.tsx                         # Landing 行銷導流 ✅（#credits 黑帶）
│   ├── pages/ComposeSamplePage.tsx               # 組裝練習／示範（0 LLM）✅
│   ├── data/composeDemoPack.ts                   # canned demo pack 三語 ✅
│   ├── components/credits/CreditPackShelf.tsx    # 點數三卡；landing 價錢 Inter／settings Cormorant ✅
│   ├── components/features/PostKitPanel.tsx      # 發文套件 L0 ✅（詳情主路 2026-09-03 不掛載）
│   ├── components/features/PostComposerPanel.tsx # 公眾卡組裝器 FB/IG/Threads；100/150/500；Keep ✅
│   ├── components/features/ImageGallery.tsx      # 精選 2×2＋下載 JPG ✅
│   ├── lib/downloadJpeg.ts                       # 瀏覽器轉 sRGB JPEG ✅
...
│   ├── compose_service.py / compose_generate.py  # JIT 組裝＋兩段 body→meta ✅
│   ├── compose_caps.py / compose_tone.py         # 100/150/500；tone cards ✅
│   ├── compose_prompt.py / compose_parse.py      # 事實錨＋Keep；hashtag 邊界 ✅
│   └── …（其餘服務略）
│
├── docs/
...
| **15 天硬刪清理** | 03:00 UTC | **現行實作**：`SchedulerService._cleanup_old_data` 用 `retention_days`（缺省／無頂層 key → **15**）；按日期批次刪主題。yaml `retention_tiers`（7～60）**尚未接線**。 |
| `POST …/schedules/generate-today` | 補齊今日 | 讀 **config count**（5×3=15）；**HKT + `generated_at`** 計數；需 `ENABLE_SCHEDULED_TOPIC_COLLECTION=true` |

**問題卡 hide（2026-09-16 · `feature/v8-topic-hide-feed`）**

| 項目 | 說明 |
|------|------|
| 政策 | 配圖錯／內容有問題 → **`hidden=true` 只隱藏**；**不清洗 Mongo 內文、不手動刪卡** |
| 列表／今日計數 | `list_topics_hidden_filter` 排除 `hidden=true` |
| 詳情 | `GET /topics/{id}` 對 hidden → **404** |
| 標記腳本 | `scripts/hide_topics.py --ids … --reason …` |
| 與 15 天清理 | **正交**：hide 不改清理；超過保留日仍會被硬刪 |

**抽取品質／沉底（2026-09-18 · `feature/v8-popbee-extract-quality` · `d03fc7b`）**

| 項目 | 說明 |
|------|------|
| 產品原則 | **寬門產卡**（易滿每日 15）＋**產後審查沉底**；Q／品質分**不作拒產門檻** |
| RSS 優先 | `article_body_resolve`：`content:encoded`／長 summary → 再 HTTP；HTTP 殼不當正文 |
| 清洗 | `article_boilerplate`：mega-menu token≥4、會員 CTA、分享列、WWD recirc、購物 cutoff |
| 產後 scan | `topic_post_scan`→`content_clean`；殼 `sort_penalty≥70`；預設不 hide |
| 列表排序 | `build_topic_list_sort`：`generated_at` 主序、penalty tie-break（**PR #75**） |
| 詳情 | 壞／殼正文 → `summary_flash`＋`topics.sourceExtractFallback`（三語） |
| 監察 log | `TOPIC_EXTRACT_RSS_FIRST`／`HTTP_SHELL_REJECT`／`QUALITY`／`TOPIC_POST_SCAN`／`TOPIC_CARD_SINK` |
| 不做 | 不回填舊卡；不上 Headless；不改 taxonomy／每日 15／翻譯核心 |
...
**流水線日誌前綴**：`[SUMMARY_FLASH_SUCCESS]`、`[I18N_CACHE_HIT]`、`[CACHE_MISS]`、`[TRANSLATION_FALLBACK_TRIGGERED]`、`[TOKEN_GATEWAY_PASSED]`、`[PUBLIC_FEED_DEV_CAP]`、`[TOPIC_EXTRACT_RSS_FIRST]`、`[TOPIC_EXTRACT_HTTP_SHELL_REJECT]`、`[TOPIC_EXTRACT_QUALITY]`、`[TOPIC_POST_SCAN]`、`[TOPIC_CARD_SINK]`
...
| 版本 | 日期 | 更新 |
|------|------|------|
| **v8.0.9-popbee-extract-quality** | 2026-09-18 | 寬門＋RSS 優先＋殼沉底＋詳情 flash 回退；`d03fc7b`／`feature/v8-popbee-extract-quality`（⏳ 待合） |
| **v8.0.8-topic-sort-penalty** | 2026-09-18 | **PR #75**：列表 `generated_at` 先、`sort_penalty` tie-break；修 Dashboard 今日卡被埋 |
| **v8.0.7-topic-hide-feed** | 2026-09-16 | 問題卡 `hidden` 過濾（列表／今日／詳情 404）；`hide_topics.py`；**不改** 15 天硬刪；分支 `feature/v8-topic-hide-feed` |
| **v8.0.6-compose-advanced** | 2026-09-16 | Compose advanced：**PR #68／#69** 合 `main`；100／150／500、Keep／`/compose-sample`；Py3.11 f-string hotfix |
| **v8.0.5c-welcome-price-merged** | 2026-09-16 | **PR #67** Welcome 點數價錢 **Inter**＋權益列合 `main` |
| **v8.0.5b-welcome-price-font** | 2026-09-16 | Welcome 點數價錢 **Inter**（`font-price`）；Settings 仍 Cormorant；`1e88287`／`feature/v8-welcome-price-font`（已合 PR #67） |
| **v8.0.5-topic-card-painpoints** | 2026-09-15 | **PR #65** 痛點 1～4：boilerplate 擴詞＋顯示層清洗；事實摘要／translate-source-article；object-top＋計數 4；下載 JPG（合 `main` `cd5a891`） |
| **v8.0.4b-credits-fulfill** | 2026-09-15 | **PR #64** 入帳對實收金額；`POST /billing/confirm`；Checkout email／session_id |
| **v8.0.4-featured-photos** | 2026-09-04 | **PR #62** 精選最多 4 張 2×2；`match_facts` 錨 `summary_flash`→原文→標題；備份 `2026-09-04_featured-photos_snapshot` |
| **v8.0.3-credits** | 2026-09-04 | **PR #59** 登入點數＋Stripe 一次三檔；正式域 02:52 UTC 15/15；備份 `2026-09-04_credits-kling_snapshot` |
| **v8.0.2-post-composer** | 2026-09-03 | **PR #58**：詳情主路 `PostComposerPanel` + `POST /alter-ego/compose`；Railway 08:07 UTC health 200、15/15 略過補產 |
