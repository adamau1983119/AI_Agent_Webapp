...
> **更新日期**：2026-08-12（**Railway env 對齊 v6**、PR #38 主題去重、Google 圖片搜尋）  
> **分支**：正式域映像追 `main`（PR #38 merge `4819e99`）；圖片搜尋 PR 待合 `feature/v8-image-search-ddgs-fix`  
...
> **日收備份**：[2026-08-12 Railway env／圖片搜尋](./docs/backups/2026-08-12_railway_env_image_ops_snapshot/SNAPSHOT_README.md) · [2026-08-11 topic HKT PR24](./docs/backups/2026-08-11_topic_dashboard_hkt_pr24_snapshot/SNAPSHOT_README.md) · [2026-08-11 ops digest](./docs/backups/2026-08-11_ops_digest_resend_pass_snapshot/SNAPSHOT_README.md)  
...
| **圖片搜尋** | v6 本機 Google + 備援 | **Google Custom Search 必備**（Railway `GOOGLE_*`）；**禁止 DuckDuckGo**（PR 待合）；v6 本機 key 在 `.env` **不會**自動上 Railway |
...
| **圖片搜尋（Topic 詳情）** | ✅ Railway 已設 `GOOGLE_API_KEY`＋`GOOGLE_SEARCH_ENGINE_ID`（2026-08-12）；API `ImageServiceManager` **Google 優先**（DuckDuckGo 移除 PR 待合） |
| **上架判決** | **已上架營運** |
...
| **已廢止** | DuckDuckGo 無 key 後備（`feature/v8-image-search-ddgs-fix`） |
