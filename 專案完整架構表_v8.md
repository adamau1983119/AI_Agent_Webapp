...
> **更新日期**：2026-08-10（**OPS-DIGEST**：正式域 loop ✅ · 自動進信箱 ❌ SMTP:587 timeout；正式解＝Pro 或 HTTPS EmailService）  
> **分支**：正式域映像追 `main`（OBS 已合）；診斷／文件當日見工作記錄  
...
> **日收備份**：[docs/backups/2026-08-10_ops_digest_smtp_block_snapshot/](./docs/backups/2026-08-10_ops_digest_smtp_block_snapshot/SNAPSHOT_README.md) · [2026-07-31](./docs/backups/2026-07-31_v8_daily_ops_ready_snapshot/SNAPSHOT_README.md) · [2026-07-30](./docs/backups/2026-07-30_v8_hosting_b_day2_snapshot/SNAPSHOT_README.md)  
...
| 項目 | v7（凍結） | v8（2026-08-10） |
|------|------------|------------------|
| 程式基線 | `9556ea7` 已合 `main`（PR #17） | OBS／digest 已合 `main`；正式域 Deploy 執行中 |
| 測試 | 上架衝刺三日主路已過 | **正式域日常運作**（CX／OPS checklist） |
| 託管 | 未接 | **方案 B**：Vercel + Railway ✅ |
| DNS／OAuth | 未接 | 前端＋API DNS／health ✅；正式域登入可用 |
| 產卡 | 預設關 | **`ENABLE_SCHEDULED_TOPIC_COLLECTION=true`**（每日 04:00 HKT × 各 5） |
| 告警／日報 | 無 | Watchdog＋digest **loop ✅**；**正式域 SMTP 寄信 ❌**（Hobby 擋 :587） |
| 文件 SoT | archives 凍結檔 | **本檔** |
| **上架判決** | — | **已上架營運**；殘餘：**OPS-DIGEST 正式域自動進信箱**（Pro 或 HTTPS EmailService） |
...
| **Observability** | loop／旗標 ✅；**自動日報進信箱 ❌**（SMTP timeout）；見下節＋[`v8_observability_alerting.md`](./docs/v8_observability_alerting.md) |
| **上架判決** | **已上架營運**（客戶路徑可用） |
| **殘餘阻塞 ≤3** | ① **正式域 `EmailService` 寄信**（Pro SMTP 或 HTTPS）；② 密鑰輪換；③ （可選）紅燈告警同路驗證 |
...
| 項目 | 內容 |
|------|------|
| **正確行為** | Railway Webapp ≥**08:00 HKT** 自動寄日報到 `OBS_OPS_EMAIL`；綠燈也寄；紅燈另即時告警 |
| **程式路徑** | `ops_watchdog` → `daily_digest` → `alert_mailer` → **`EmailService`（Gmail SMTP:587）** |
| **現況（2026-08-10）** | `WATCHDOG_START digest=True` ✅；`DIGEST_TICK digest_failed` ❌＝`Timed out connecting to smtp.gmail.com on port 587` |
| **本機** | `python scripts/send_obs_daily_digest_now.py` 可 PASS（僅證 Gmail／程式，**≠**正式自動） |
| **正式解** | ① Railway **Pro**（開出站 SMTP）Redeploy；或 ② 改 `EmailService` **HTTPS** 寄信 |
| **禁止** | 本機排程／GitHub Actions 充當正式營運寄信 |
| **Checklist** | [`v8_observability_alerting.md`](./docs/v8_observability_alerting.md) **PD-OBS-TL-07b `[!]`** · [`v8_daily_ops_cx_checklist.md`](./docs/v8_daily_ops_cx_checklist.md) **OPS-DIGEST** |
...
| 路徑 | 元件 | v7_nav | 說明 |
|------|------|:------:|------|
| `/` | RootRedirect | — | token→**pending? onboarding : MyChannel**；无语言→`/language`；未登入→**`/welcome`**（v7.1 Landing） |
| `/onboarding/alter-ego` | AlterEgoOnboarding | **顯示** | 首登范文；含 **Skip** → `/my-channel` |
| `/language` | LanguageSelection | 公開 | |
| `/login` … `/privacy` | 認證／靜態 | 公開 | 同 v6 |
| `/my-channel` | MyChannel | **顯示** | 登入預設首屏；專屬 feed + 點數解鎖（MC-1～6） |
| `/dashboard` | Dashboard | **顯示** | v7.1 可 redirect→`/my-channel` 或改寫 |
| `/topics` | Topics | **顯示** | |
...
| 排程 | v6 描述 | v7 正式／本機測試 |
|------|---------|-------------------|
| 主打三類 `fashion/food/trend` | 每 6h × 各 10 | **預設關**（`ENABLE_SCHEDULED_TOPIC_COLLECTION=false`）。**測試／日更**：`topic_generation.yaml` → `mode: daily`、每類 **count=5**（07:00 HKT）；翻譯另受 `ENABLE_AI_TOPIC_TRANSLATION` |
...
| `POST …/schedules/generate-today` | 補齊今日 | 讀 **config count**（現為 5×3=15）；需 `ENABLE_SCHEDULED_TOPIC_COLLECTION=true` |
