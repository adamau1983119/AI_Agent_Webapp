...
> **更新日期**：2026-08-11（**PR #24**：Dashboard 主題卡 HKT filter、triple preload、Dashboard-first 登入）  
> **分支**：正式域映像追 `main`（PR #24 merge `b60a285`）  
...
> **日收備份**：[docs/backups/2026-08-11_topic_dashboard_hkt_pr24_snapshot/](./docs/backups/2026-08-11_topic_dashboard_hkt_pr24_snapshot/SNAPSHOT_README.md) · [2026-08-11_ops_digest_resend_pass_snapshot/](./docs/backups/2026-08-11_ops_digest_resend_pass_snapshot/SNAPSHOT_README.md) · [2026-07-31](./docs/backups/2026-07-31_v8_daily_ops_ready_snapshot/SNAPSHOT_README.md) · [2026-07-30](./docs/backups/2026-07-30_v8_hosting_b_day2_snapshot/SNAPSHOT_README.md)  
...
|------|------------|------------------|
| 程式基線 | `9556ea7` 已合 `main`（PR #17） | OBS＋PR #19 Resend＋**PR #24 主題卡 HKT** 已合 `main` |
| 測試 | 上架衝刺三日主路已過 | **正式域日常運作**（CX／OPS）；**OPS-CARD 修復已部署** |
| 託管 | 未接 | **方案 B**：Vercel + Railway ✅ |
| DNS／OAuth | 未接 | 前端＋API DNS／health ✅；Resend 網域 Verified ✅ |
| 產卡 | 預設關 | **`ENABLE_SCHEDULED_TOPIC_COLLECTION=true`**（每日 **04:00 HKT** × 各 5 = **15**） |
...
| 三語產後預載 | `backend/app/services/automation/topic_triple_preload.py` | DeepL en/ja（preload_languages_for）；cap=`TOPIC_TRIPLE_PRELOAD_CAP`；語言 SoT 以 `backend/config/topic_languages.json` 為主（Railway hotfix：避免 repo root `shared/` 路徑缺失） |
